             ...
             <div class="card m-auto col-12">
                ...
                <div class="card-body">
                    <table class="table table-success table-striped">
                        ...
                        <tbody>
                     <?php
                    $selects="SELECT * FROM fmenu ORDER BY id DESC";//select forentend services 
                    $db_result_s=mysqli_query($db_connect,$selects);
                    foreach($db_result_s as $key=>$menu):
                    ?>
                        <tr>
                          <td><?=$key+1?></td>
                          <td><?=$menu['menu']?></td>
                          <td><a href="logo_status_toggle.php?id=<?=$menu['id']?>&status=<?=$menu['status']?>"><?=$menu['status']==1?'<i style="font-size:40px;color:purple;" class="flaticon-381-switch-4"></i>':
                          '<i style="font-size:40px;color:red;" class="flaticon-381-switch-5"></i>'?></a>
                          <td>
